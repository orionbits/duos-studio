...
import { getAnswerPreview, isCorrectStringAnswer, joinClasses } from "@/components/exercises/helpers";
import type { ExerciseProps, ShortAnswerData } from "@/components/exercises/types";
  ...
  return (
    <section className="rounded-[2rem] border border-white/10 bg-white/[0.02] p-8 backdrop-blur-3xl shadow-2xl overflow-hidden relative">
      <div className="absolute top-0 right-0 p-8 opacity-10 pointer-events-none">
        <span className="text-6xl font-black italic tracking-tighter text-white">EXERCISE</span>
      </div>
      ...
      <div className="space-y-6 relative z-10">
        {data.map((item, index) => {
          const answerKey = `${id}-${index + 1}`;
          const value = answers[answerKey] ?? "";
          const isCorrect = showFeedback && isCorrectStringAnswer(value, item.answer);

          return (
            <div key={answerKey} className="group rounded-2xl border border-white/5 bg-black/20 p-6 transition-all hover:border-white/10">
              <p className="mb-4 text-sm font-medium text-slate-400 group-hover:text-white transition-colors">
                <span className="text-teal-500/50 mr-2">{String(index + 1).padStart(2, '0')}</span> {item.question}
              </p>
              <input
                value={value}
                onChange={(event) => handleChange(answerKey, event.target.value)}
              ...
              {(showFeedback || revealed) && (
                <div className="mt-4 flex items-center gap-2 rounded-lg bg-white/5 border border-white/10 px-4 py-2">
                  <span className="text-[10px] font-bold uppercase tracking-widest text-slate-500">Correct Answer:</span>
