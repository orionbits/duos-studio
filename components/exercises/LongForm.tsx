...
import type { ExerciseProps, LongFormData } from "@/components/exercises/types";
  ...
  return (
    <section className="rounded-[2rem] border border-white/10 bg-white/[0.02] p-8 backdrop-blur-3xl shadow-2xl overflow-hidden relative">
      <div className="absolute top-0 right-0 p-8 opacity-10 pointer-events-none">
        <span className="text-6xl font-black italic tracking-tighter text-white">WRITE</span>
      </div>
      ...
      <div className="space-y-4 relative z-10">
        <p className="text-base font-medium text-slate-300">{data.prompt}</p>
        <textarea
          value={answer}
          onChange={(event) => handleChange(event.target.value)}
          rows={8}
          className="w-full rounded-2xl border border-white/10 bg-black/40 px-6 py-5 text-sm text-white outline-none focus:border-indigo-500/50 focus:ring-1 focus:ring-indigo-500/50 transition-all font-light leading-relaxed"
      ...
      {(showFeedback || revealed) && (
        <div className="mt-8 rounded-2xl border border-white/10 bg-black/40 p-6 relative z-10 animate-in fade-in slide-in-from-bottom-2">
          <p className="mb-2 text-[10px] font-black uppercase tracking-widest text-slate-500">Suggested Solution:</p>
